...
class PageAdmin(model_admin):
    ...
    def get_urls(self):
        ...
        url_patterns = patterns('',
            ...
            pat(r'^(?P<page_id>\d+)/add-plugin/(?P<placeholder_id>\w+)/(?P<plugin_type>\w+)/(?P<language>\w+)/$', self.add_plugin),
    ...
    def add_plugin(self, request, page_id, placeholder_id, plugin_type, language):
        ...
        if request.method == "POST":#Comment needed : how could not be POST ?
            #Define main values
            #page = get_object_or_404(Page, pk=page_id)
            ...
            # page add-plugin
            if page:
                #TODO : clarify how comes the language value
                if language is None:
