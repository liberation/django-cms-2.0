...
/*for(id in placeholder_data)
{
    placeholder = placeholder_data[id]
    for(idd in placeholder)
    {
        plugin = placeholder[idd]
        alert(plugin)
        break;
    }
}*/
...
function move(direction){
	...
	$.post(cms_urls['cms_page_move_plugin'], { ids:result.join("_") }, function(data){
...
$(document).ready(function () {
	...
	$('.cms_toolbar_plugintools .cms_toolbar_button').each(function(){
		$(this).attr('title', $(this).find('strong').text());
	});
	...
	// reduced plugins
	$('a[id^=reduced_button]').click(function(event) {
	...
	$('.cms_placeholder').sortable({
		...
		update: function(event, ui) {
			var ids = [];
			$(this).sortable('toArray').each(function(dom_id) {
